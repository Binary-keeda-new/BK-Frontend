...
for (const [key, filepath] of Object.entries(paths)) {
    if (fs.existsSync(filepath)) {
        ...
        try {
            ...
            newMcqString += `  "${key}": ` + JSON.stringify(arr, null, 4).split('\n').map((l, i) => i === 0 ? l : '  ' + l).join('\n') + ",\n";
        } catch(e) {
...
let debugStart = content.indexOf('export const DEBUG: Record<string, DebugExercise> = {');

if (mcqStart !== -1 && debugStart !== -1) {
    content = content.substring(0, mcqStart) + newMcqString + content.substring(debugStart);
