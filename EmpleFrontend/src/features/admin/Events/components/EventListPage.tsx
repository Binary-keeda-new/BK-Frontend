...
import { Trophy, Sparkles, Rocket, Pencil, Trash2, Plus, CalendarDays } from 'lucide-react'
...
export default function EventListPage({ type }: { type: EventType }) {
    ...
    <div className="p-7">
      ...
      <div className="mb-6 flex items-start justify-between">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <div
              className={`w-7 h-7 rounded-lg ${config.iconBg} flex items-center justify-center`}
          ...
          <p className="text-white/40 text-sm">
            Manage all {config.label.toLowerCase()} visible to users.
          </p>
        </div>
        <button
      ...
      {loading ? (
        <div className="flex flex-col gap-3">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-24 rounded-2xl bg-[rgb(19,20,27)] animate-pulse ring-1 ring-white/5" />
          ))}
        </div>
      ) : events.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">
          {events.map((event) => (
            <div
              key={event._id}
              className="flex items-start justify-between gap-4 rounded-2xl bg-[rgb(19,20,27)] px-5 py-4 ring-1 ring-white/10 hover:ring-white/15 transition"
            >
              <div className="flex-1 min-w-0">
                ...
                <p className="text-white/40 text-xs truncate mb-1">{event.description}</p>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <button
                  ...
                  className="rounded-xl bg-[rgb(10,11,14)] p-2 text-white/50 ring-1 ring-white/10 transition hover:text-white"
                >
                  <Pencil size={14} />
                </button>
                <button
                  onClick={() => handleDelete(event._id)}
                  className="rounded-xl bg-[rgb(10,11,14)] p-2 text-red-400/50 ring-1 ring-white/10 transition hover:text-red-400"
                >
                  <Trash2 size={14} />
                </button>
              </div>
