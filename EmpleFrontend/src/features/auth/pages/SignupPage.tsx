...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('signup sync ->', syncRes.status, syncText)

      if (!syncRes.ok) {
        setError(`Signup succeeded, but user sync failed: ${syncRes.status} ${syncText}`)
        return
      }
      ...
      <div className="auth-card-wrap" style={{ maxWidth: 600 }}>
        <div className="auth-card">
          ...
          <form className="auth-form" onSubmit={handleSubmit}>
            ...
            <div className="auth-field">
              <label className="auth-label">
                Password <span style={{ color: 'var(--clr-accent)' }}>*</span>
              </label>
              <div className="auth-input-wrap">
                <input
                ...
                <div className="auth-strength">
                  <div className="auth-strength-bars">
                    {[1, 2, 3, 4].map((n) => (
                      <div
                        key={n}
                        className={`auth-strength-bar${strength.level >= n ? ` ${strength.color}` : ''}`}
                      />
                    ))}
                  </div>
            ...
            <div className="auth-field">
              <label className="auth-label">
                Confirm Password <span style={{ color: 'var(--clr-accent)' }}>*</span>
              </label>
              <div className="auth-input-wrap">
                <input
                  className="auth-input has-toggle"
              ...
              {form.confirm && form.confirm === form.password && (
                <div style={{ fontSize: 'var(--text-xs)', color: '#22c55e', marginTop: 4 }}>
                  ✓ Passwords match
                </div>
              )}
            </div>
