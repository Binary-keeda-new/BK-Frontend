/**
 * Placement Roadmap (293 Days)
 * TypeScript Data Structure
 */

const rawDays = [
  {
    "day": 1,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Complete playlist of Career Ride YouTube channel and make notes"
      },
      {
        "category": "Core Subjects",
        "topic": "Dont Touch this section for now"
      }
    ],
    "resources": [
      {
        "type": "text",
        "title": "Carrer Ride",
        "url": null
      }
    ]
  },
  {
    "day": 2,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 3,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 4,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 5,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 6,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 7,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 8,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 9,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 10,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 11,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 12,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 13,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 14,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 15,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 16,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 17,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 18,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 19,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 20,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 21,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 22,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 23,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 24,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 25,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 26,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 27,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 28,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "Core Subjects",
        "topic": "DBMS for placements"
      }
    ],
    "resources": [
      {
        "type": "text",
        "title": "DBMS Lec: DBMS",
        "url": null
      },
      {
        "type": "video",
        "title": "MCQ Practice",
        "url": "https://www.indiabix.com/online-test/aptitude-test/"
      },
      {
        "type": "text",
        "title": "DBMS Interview Questions:",
        "url": null
      },
      {
        "type": "video",
        "title": "Video Link",
        "url": "https://www.geeksforgeeks.org/commonly-asked-dbms-interview-questions/"
      }
    ]
  },
  {
    "day": 29,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 30,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 31,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 32,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 33,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 34,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude Practice"
      }
    ],
    "resources": []
  },
  {
    "day": 35,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 36,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 37,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 38,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 39,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 40,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 41,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude Practice"
      }
    ],
    "resources": []
  },
  {
    "day": 42,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "Core Subjects",
        "topic": "OS for placements"
      }
    ],
    "resources": [
      {
        "type": "text",
        "title": "OS Lec: OS",
        "url": null
      },
      {
        "type": "video",
        "title": "MCQ Practice",
        "url": "https://www.indiabix.com/online-test/aptitude-test/"
      },
      {
        "type": "text",
        "title": "OS Interview Questions:",
        "url": null
      },
      {
        "type": "video",
        "title": "Video Link",
        "url": "https://www.geeksforgeeks.org/operating-systems-interview-questions/"
      }
    ]
  },
  {
    "day": 43,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 44,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 45,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 46,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 47,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 48,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude Practice"
      }
    ],
    "resources": []
  },
  {
    "day": 49,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "Core Subjects",
        "topic": "CN for placements"
      }
    ],
    "resources": [
      {
        "type": "text",
        "title": "CN Lec: CN",
        "url": null
      },
      {
        "type": "video",
        "title": "MCQ Practice",
        "url": "https://www.indiabix.com/online-test/aptitude-test/"
      },
      {
        "type": "text",
        "title": "CN Interview Questions:",
        "url": null
      },
      {
        "type": "video",
        "title": "Video Link",
        "url": "https://www.interviewbit.com/networking-interview-questions/"
      }
    ]
  },
  {
    "day": 50,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 51,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 52,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 53,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 54,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 55,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude Practice"
      }
    ],
    "resources": []
  },
  {
    "day": 56,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "Tools",
        "topic": "Agile Basics"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "Agile",
        "url": "https://youtu.be/WjwEh15M5Rw?feature=shared"
      },
      {
        "type": "video",
        "title": "SQL",
        "url": "https://youtu.be/hlGoQC332VM?feature=shared"
      },
      {
        "type": "video",
        "title": "PowerBI",
        "url": "https://youtu.be/6cV3OwFrOkk?feature=shared"
      },
      {
        "type": "video",
        "title": "Linux",
        "url": "https://youtu.be/_tCY-c-sPZc?feature=shared"
      },
      {
        "type": "video",
        "title": "Git",
        "url": "https://youtu.be/Ez8F0nW6S-w?feature=shared"
      },
      {
        "type": "video",
        "title": "Numpy",
        "url": "https://youtu.be/Rbh1rieb3zc?feature=shared"
      },
      {
        "type": "video",
        "title": "Pandas",
        "url": "https://youtu.be/RhEjmHeDNoA?feature=shared"
      },
      {
        "type": "video",
        "title": "DBMS MCQ",
        "url": "https://www.interviewbit.com/dbms-mcq/"
      }
    ]
  },
  {
    "day": 57,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 58,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "Tools",
        "topic": "SQL"
      }
    ],
    "resources": []
  },
  {
    "day": 59,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 60,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 61,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 62,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "OS Revision"
      }
    ],
    "resources": []
  },
  {
    "day": 63,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "Tools",
        "topic": "Power BI"
      }
    ],
    "resources": []
  },
  {
    "day": 64,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 65,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "Tools",
        "topic": "Linux and Git"
      }
    ],
    "resources": []
  },
  {
    "day": 66,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 67,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 68,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "Tools",
        "topic": "Numpy"
      }
    ],
    "resources": []
  },
  {
    "day": 69,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "DBMS Revision and MCQ"
      },
      {
        "category": "Tools",
        "topic": "Pandas"
      }
    ],
    "resources": []
  },
  {
    "day": 70,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "Programming Language",
        "topic": "C++ Complete"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "C++ Tutorial",
        "url": "https://youtu.be/FpfHmAkRVK4?feature=shared"
      },
      {
        "type": "video",
        "title": "C++ STL",
        "url": "https://youtu.be/RRVYpIET_RU?feature=shared"
      },
      {
        "type": "video",
        "title": "Python Basics",
        "url": "https://youtu.be/vLqTf2b6GZw?feature=shared"
      },
      {
        "type": "video",
        "title": "CN MCQ",
        "url": "https://www.interviewbit.com/computer-network-mcq/"
      },
      {
        "type": "video",
        "title": "Oops in C++",
        "url": "https://www.youtube.com/watch?v=mlIUKyZIUUU"
      }
    ]
  },
  {
    "day": 71,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 72,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 73,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 74,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 75,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 76,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "CN Revision and MCQ"
      },
      {
        "category": "Tools",
        "topic": "Tools Revision"
      }
    ],
    "resources": []
  },
  {
    "day": 77,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 78,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 79,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 80,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 81,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "Programming Language",
        "topic": "Python Basics"
      }
    ],
    "resources": []
  },
  {
    "day": 82,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 83,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "OS Revision"
      }
    ],
    "resources": []
  },
  {
    "day": 84,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 85,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 86,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 87,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "Programming Language",
        "topic": "Oops in C++"
      }
    ],
    "resources": []
  },
  {
    "day": 88,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 89,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 90,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "DBMS Revision and MCQ"
      }
    ],
    "resources": []
  },
  {
    "day": 91,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "Programming Language",
        "topic": "C++ STL"
      }
    ],
    "resources": []
  },
  {
    "day": 92,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 93,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 94,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "HTML",
        "url": "https://youtu.be/HcOc7P5BMi4?feature=shared"
      },
      {
        "type": "text",
        "title": "DSA Questions Link:",
        "url": null
      }
    ]
  },
  {
    "day": 95,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 96,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 97,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "CN Revision and MCQ"
      },
      {
        "category": "Full Stack",
        "topic": "HTML"
      }
    ],
    "resources": []
  },
  {
    "day": 98,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "CSS",
        "url": "https://youtu.be/ESnrn1kAD4E?feature=shared"
      }
    ]
  },
  {
    "day": 99,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 100,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 101,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 102,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 103,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 104,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "CN Revision and MCQ"
      },
      {
        "category": "Programming Language",
        "topic": "Oops Revision"
      },
      {
        "category": "Full Stack",
        "topic": "CSS"
      }
    ],
    "resources": []
  },
  {
    "day": 105,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "CSS",
        "url": "https://youtu.be/ESnrn1kAD4E?feature=shared"
      }
    ]
  },
  {
    "day": 106,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 107,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 108,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 109,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 110,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 111,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "OS Revision"
      },
      {
        "category": "Tools",
        "topic": "Tools Revision"
      },
      {
        "category": "Full Stack",
        "topic": "CSS"
      }
    ],
    "resources": []
  },
  {
    "day": 112,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "CSS",
        "url": "https://youtu.be/ESnrn1kAD4E?feature=shared"
      }
    ]
  },
  {
    "day": 113,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 114,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 115,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 116,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 117,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 118,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "DBMS Revision and MCQ"
      },
      {
        "category": "Programming Language",
        "topic": "Python Revision"
      },
      {
        "category": "Full Stack",
        "topic": "CSS"
      }
    ],
    "resources": []
  },
  {
    "day": 119,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "CSS",
        "url": "https://youtu.be/ESnrn1kAD4E?feature=shared"
      }
    ]
  },
  {
    "day": 120,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 121,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 122,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 123,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 124,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 125,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "CN Revision and MCQ"
      },
      {
        "category": "Full Stack",
        "topic": "CSS"
      }
    ],
    "resources": []
  },
  {
    "day": 126,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "JS",
        "url": "https://youtu.be/rfObCuGLSek?feature=shared"
      }
    ]
  },
  {
    "day": 127,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 128,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 129,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 130,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 131,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 132,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "OS Revision"
      },
      {
        "category": "Tools",
        "topic": "Tools Revision"
      },
      {
        "category": "Full Stack",
        "topic": "JS"
      }
    ],
    "resources": []
  },
  {
    "day": 133,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "JS",
        "url": "https://youtu.be/rfObCuGLSek?feature=shared"
      }
    ]
  },
  {
    "day": 134,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 135,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 136,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 137,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 138,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 139,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "DBMS Revision and MCQ"
      },
      {
        "category": "Full Stack",
        "topic": "JS"
      }
    ],
    "resources": []
  },
  {
    "day": 140,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "JS",
        "url": "https://youtu.be/rfObCuGLSek?feature=shared"
      }
    ]
  },
  {
    "day": 141,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 142,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 143,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 144,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 145,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 146,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "CN Revision and MCQ"
      },
      {
        "category": "Tools",
        "topic": "Tools Revision"
      },
      {
        "category": "Programming Language",
        "topic": "Oops Revision"
      },
      {
        "category": "Full Stack",
        "topic": "JS"
      }
    ],
    "resources": []
  },
  {
    "day": 147,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "JS",
        "url": "https://youtu.be/rfObCuGLSek?feature=shared"
      }
    ]
  },
  {
    "day": 148,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 149,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 150,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 151,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 152,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 153,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "OS Revision"
      },
      {
        "category": "Full Stack",
        "topic": "JS"
      }
    ],
    "resources": []
  },
  {
    "day": 154,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "JS",
        "url": "https://youtu.be/rfObCuGLSek?feature=shared"
      }
    ]
  },
  {
    "day": 155,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 156,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 157,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 158,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 159,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 160,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "DBMS Revision and MCQ"
      },
      {
        "category": "Full Stack",
        "topic": "JS"
      }
    ],
    "resources": []
  },
  {
    "day": 161,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "JS",
        "url": "https://youtu.be/rfObCuGLSek?feature=shared"
      }
    ]
  },
  {
    "day": 162,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 163,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 164,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 165,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 166,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 167,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "OS Revision"
      },
      {
        "category": "Programming Language",
        "topic": "Python Revision"
      },
      {
        "category": "Full Stack",
        "topic": "JS"
      }
    ],
    "resources": []
  },
  {
    "day": 168,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 169,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 170,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 171,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 172,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 173,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 174,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "CN Revision and MCQ"
      },
      {
        "category": "Full Stack",
        "topic": "JS"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "JS",
        "url": "https://youtu.be/rfObCuGLSek?feature=shared"
      }
    ]
  },
  {
    "day": 175,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 176,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 177,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 178,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 179,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 180,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 181,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude and Core MCQ Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "DBMS Revision and MCQ"
      },
      {
        "category": "Tools",
        "topic": "Tools Revision"
      },
      {
        "category": "Full Stack",
        "topic": "JS"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "JS",
        "url": "https://youtu.be/rfObCuGLSek?feature=shared"
      }
    ]
  },
  {
    "day": 182,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 183,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 184,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 185,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 186,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 187,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 188,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude and Core MCQ Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "OS Revision"
      },
      {
        "category": "Full Stack",
        "topic": "Angular"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "Angular",
        "url": "https://youtu.be/JWhRMyyF7nc?feature=shared"
      }
    ]
  },
  {
    "day": 189,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 190,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 191,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 192,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 193,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 194,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 195,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude and Core MCQ Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "CN Revision and MCQ"
      },
      {
        "category": "Programming Language",
        "topic": "Python Revision"
      },
      {
        "category": "Full Stack",
        "topic": "Angular"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "Angular",
        "url": "https://youtu.be/JWhRMyyF7nc?feature=shared"
      }
    ]
  },
  {
    "day": 196,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 197,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 198,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 199,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 200,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 201,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 202,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude and Core MCQ Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "DBMS Revision and MCQ"
      },
      {
        "category": "Full Stack",
        "topic": "Angular"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "Angular",
        "url": "https://youtu.be/JWhRMyyF7nc?feature=shared"
      }
    ]
  },
  {
    "day": 203,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 204,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 205,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 206,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 207,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 208,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 209,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude and Core MCQ Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "OS Revision"
      },
      {
        "category": "Programming Language",
        "topic": "Oops Revision"
      },
      {
        "category": "Full Stack",
        "topic": "Angular"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "Angular",
        "url": "https://youtu.be/JWhRMyyF7nc?feature=shared"
      }
    ]
  },
  {
    "day": 210,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 211,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 212,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 213,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 214,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 215,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 216,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude and Core MCQ Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "DBMS Revision and MCQ"
      },
      {
        "category": "Tools",
        "topic": "Tools Revision"
      },
      {
        "category": "Full Stack",
        "topic": "Angular"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "Angular",
        "url": "https://youtu.be/JWhRMyyF7nc?feature=shared"
      }
    ]
  },
  {
    "day": 217,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 218,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 219,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 220,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 221,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 222,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 223,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude and Core MCQ Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "OS Revision and MCQ"
      },
      {
        "category": "Full Stack",
        "topic": "MongoDB"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "MongoDB",
        "url": "https://youtu.be/J6mDkcqU_ZE?feature=shared"
      }
    ]
  },
  {
    "day": 224,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 225,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 226,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 227,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 228,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 229,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 230,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude and Core MCQ Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "DBMS Revision and MCQ"
      },
      {
        "category": "Programming Language",
        "topic": "Python Revision"
      },
      {
        "category": "Full Stack",
        "topic": "Node / Express JS"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "NodeJS",
        "url": "https://youtu.be/7fjOw8ApZ1I?feature=shared"
      }
    ]
  },
  {
    "day": 231,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 232,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 233,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 234,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 235,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 236,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 237,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude and Core MCQ Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "CN Revision and MCQ"
      },
      {
        "category": "Tools",
        "topic": "Tools Revision"
      },
      {
        "category": "Full Stack",
        "topic": "Node / Express JS"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "NodeJS",
        "url": "https://youtu.be/7fjOw8ApZ1I?feature=shared"
      }
    ]
  },
  {
    "day": 238,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 239,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 240,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 241,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 242,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 243,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 244,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude and Core MCQ Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "DBMS Revision and MCQ"
      },
      {
        "category": "Full Stack",
        "topic": "Node / Express JS"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "NodeJS",
        "url": "https://youtu.be/7fjOw8ApZ1I?feature=shared"
      }
    ]
  },
  {
    "day": 245,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 246,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 247,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 248,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 249,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 250,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 251,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude and Core MCQ Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "OS Revision and MCQ"
      },
      {
        "category": "Programming Language",
        "topic": "Oops Revision"
      },
      {
        "category": "Full Stack",
        "topic": "Node / Express JS"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "NodeJS",
        "url": "https://youtu.be/7fjOw8ApZ1I?feature=shared"
      }
    ]
  },
  {
    "day": 252,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 253,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 254,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 255,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 256,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 257,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 258,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude and Core MCQ Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "DBMS Revision and MCQ"
      },
      {
        "category": "Programming Language",
        "topic": "Python Revision"
      },
      {
        "category": "Full Stack",
        "topic": "Node / Express JS"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "NodeJS",
        "url": "https://youtu.be/7fjOw8ApZ1I?feature=shared"
      }
    ]
  },
  {
    "day": 259,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 260,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 261,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 262,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 263,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 264,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 265,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude and Core MCQ Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "CN Revision and MCQ"
      },
      {
        "category": "Tools",
        "topic": "Tools Revision"
      },
      {
        "category": "Full Stack",
        "topic": "Node / Express JS"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "NodeJS",
        "url": "https://youtu.be/7fjOw8ApZ1I?feature=shared"
      }
    ]
  },
  {
    "day": 266,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 267,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 268,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 269,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 270,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 271,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 272,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude and Core MCQ Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "OS Revision and MCQ"
      },
      {
        "category": "Full Stack",
        "topic": "Node / Express JS"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "NodeJS",
        "url": "https://youtu.be/7fjOw8ApZ1I?feature=shared"
      }
    ]
  },
  {
    "day": 273,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 274,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 275,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 276,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 277,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 278,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 279,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Aptitude and Core MCQ Practice"
      },
      {
        "category": "Core Subjects",
        "topic": "CN Revision and MCQ"
      },
      {
        "category": "Tools",
        "topic": "Puzzles Revision"
      },
      {
        "category": "Full Stack",
        "topic": "Node / Express JS"
      }
    ],
    "resources": [
      {
        "type": "video",
        "title": "NodeJS",
        "url": "https://youtu.be/7fjOw8ApZ1I?feature=shared"
      },
      {
        "type": "video",
        "title": "Puzzles",
        "url": "https://www.geeksforgeeks.org/top-20-puzzles-commonly-asked-during-sde-interviews/"
      }
    ]
  },
  {
    "day": 280,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 281,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 282,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 283,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 284,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 285,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "DSA",
        "topic": "DSA"
      }
    ],
    "resources": []
  },
  {
    "day": 286,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Aptitude",
        "topic": "Revise Everything"
      }
    ],
    "resources": []
  },
  {
    "day": 287,
    "dayOfWeek": "Monday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 288,
    "dayOfWeek": "Tuesday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 289,
    "dayOfWeek": "Wednesday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 290,
    "dayOfWeek": "Thursday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 291,
    "dayOfWeek": "Friday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 292,
    "dayOfWeek": "Saturday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  },
  {
    "day": 293,
    "dayOfWeek": "Sunday",
    "targets": [
      {
        "category": "Rest/Revision",
        "topic": "Rest and revision of previous topics"
      }
    ],
    "resources": []
  }
];

export const placementRoadmap = {
  id: 'placement-roadmap',
  title: 'Placement Roadmap',
  name: 'Placement Roadmap',
  description: 'Comprehensive 293-day structured guide covering Aptitude, Core Subjects, Tools, Coding, and DSA.',
  difficulty: 'Intermediate',
  icon: '📅',
  color: 'purple',
  domain: 'Placement Preparation',
  category: 'Technical',
  enrolled: 4892,
  totalSections: 293,
  targetUsers: ['Global BK', 'Campus'],
  sections: rawDays.map(d => ({ ...d, id: d.day })),
  days: rawDays
};

const getPlacementRoadmap = () => placementRoadmap;
export default getPlacementRoadmap;
