...
export default function CTutorialPage({ onBack }: CTutorialPageProps) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("c_tutorial_progress");
    if (saved) {
      try {
  ...
  const addXP = (pts: number) => {
    setCompletedMap(m => {
      ...
      localStorage.setItem("c_tutorial_progress", JSON.stringify(newMap));
      return newMap;
    });
  };

  const resetProgress = () => {
    if (window.confirm("Are you sure you want to reset all your C Programming tutorial progress?")) {
      setCompletedMap({});
      localStorage.removeItem("c_tutorial_progress");
    }
  };
