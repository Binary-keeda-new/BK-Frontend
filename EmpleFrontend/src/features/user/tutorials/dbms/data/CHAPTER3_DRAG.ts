export const CHAPTER3_DRAG = [
  {
    instructions: "Arrange the SQL execution order for a complex query on Customers.",
    lines: [
      { id: "6", text: "ORDER BY column" },
      { id: "4", text: "HAVING condition" },
      { id: "1", text: "FROM Customers" },
      { id: "3", text: "GROUP BY column" },
      { id: "2", text: "WHERE condition" },
      { id: "5", text: "SELECT columns" }
    ],
    order: ["1", "2", "3", "4", "5", "6"]
  },
  {
    instructions: "Arrange the typical transaction lifecycle for Data Models.",
    lines: [
      { id: "1", text: "BEGIN TRANSACTION" },
      { id: "2", text: "UPDATE Products SET ..." },
      { id: "3", text: "Log changes to write-ahead log" },
      { id: "4", text: "COMMIT TRANSACTION" }
    ],
    order: ["1", "2", "3", "4"]
  }
];