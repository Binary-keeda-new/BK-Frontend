export const CHAPTER9_DRAG = [
  {
    instructions: "Arrange the logical steps to design the Inventory schema for Relational Calculus.",
    lines: [
      { id: "1", text: "Gather Requirements" },
      { id: "2", text: "Create ER Diagram" },
      { id: "3", text: "Normalize to 3NF" },
      { id: "4", text: "Write CREATE TABLE statements" },
      { id: "5", text: "Create Indexes" }
    ],
    order: ["1", "2", "3", "4", "5"]
  },
  {
    instructions: "Arrange the typical transaction lifecycle for Relational Calculus.",
    lines: [
      { id: "4", text: "COMMIT TRANSACTION" },
      { id: "3", text: "Log changes to write-ahead log" },
      { id: "2", text: "UPDATE Employees SET ..." },
      { id: "1", text: "BEGIN TRANSACTION" }
    ],
    order: ["1", "2", "3", "4"]
  }
];