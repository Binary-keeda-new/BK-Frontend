...
// The issue is that last time we replaced, we put a literal '\n' which means indexOf '};' might be messed up.
// Let's find the end of the MCQ object more carefully.
// The next export is DEBUG_EXERCISES (or we can just find '};\n\nexport const DEBUG_EXERCISES' or similar).
// Let's look for 'export const COMPLETE_EXERCISES' or whatever the next one is.
let nextExportStart = content.indexOf('export const COMPLETE_EXERCISES', mcqStart);
if (nextExportStart === -1) nextExportStart = content.indexOf('export const', mcqStart + 50);

if (mcqStart !== -1 && nextExportStart !== -1) {
    // We want everything up to mcqStart, then newMcqString, then whatever is from nextExportStart onwards.
    // Wait! The previous file had:
    // export const MCQ = { ... }
    // export const COMPLETE_EXERCISES = { ... }
    content = content.substring(0, mcqStart) + newMcqString + '\n' + content.substring(nextExportStart);
    fs.writeFileSync(targetFile, content);
    console.log("Successfully fixed cTutorial.ts!");
} else {
    console.error("Could not find boundaries in cTutorial.ts");
}
